...
namespace nt2
{
  namespace tag
  {
    ...
    **/
    struct var_ : ext::abstract_<var_>
    {
      /// @brief Parent hierarchy
       typedef ext::abstract_<var_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_var_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site, class... Ts>
    BOOST_FORCEINLINE generic_dispatcher<tag::var_, Site> dispatching_var_(adl_helper, boost::dispatch::meta::unknown_<Site>, boost::dispatch::meta::unknown_<Ts>...)
    {
      return generic_dispatcher<tag::var_, Site>();
    }
    template<class... Args>
    struct impl_var_;
  }
