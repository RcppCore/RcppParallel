...
namespace nt2
{
  namespace tag
  {
    struct idivide_ : ext::abstract_<idivide_>
    {
      typedef ext::abstract_<idivide_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_idivide_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site, class... Ts>
    BOOST_FORCEINLINE generic_dispatcher<tag::idivide_, Site> dispatching_idivide_(adl_helper, boost::dispatch::meta::unknown_<Site>, boost::dispatch::meta::unknown_<Ts>...)
    {
      return generic_dispatcher<tag::idivide_, Site>();
    }
    template<class... Args>
    struct impl_idivide_;
  }
