...
namespace nt2
{
  namespace tag
  {
    namespace factorization
    {
      struct poly_ : ext::unspecified_<factorization::poly_>
      {
        ...
        BOOST_AUTO_DECLTYPE_HEADER( dispatching( ext::adl_helper(), *(poly_*)0, static_cast<Args&&>(args)... ) )
        {
          return dispatching( ext::adl_helper(), poly_(), static_cast<Args&&>(args)... );
        }
      };
    }
  ...
  namespace ext
  {
    ...
    BOOST_FORCEINLINE generic_dispatcher<tag::poly_, Site> dispatching_poly_(adl_helper, boost::dispatch::meta::unknown_<Site>, boost::dispatch::meta::unknown_<Ts>...)
    {
      return generic_dispatcher<tag::poly_, Site>();
    }
    template<class... Args>
    struct impl_poly_;
  }
