...
{
  BOOST_DISPATCH_IMPLEMENT  ( erfcinv_, tag::cpu_
                            , (A0)(X)
                            , ((simd_<single_<A0>,X>))
                            )
  {
    ...
    NT2_FUNCTOR_CALL(1)
    {
      ...
      w = if_else(test, w-Two5, sqrt(w));
      std::size_t nb = inbtrue(test);
      A0 r1;
      if(nb > 0)
      {
        r1 = a00*details::erfinv_kernel<A0>::erfinv1(w);
        if(nb >= meta::cardinal_of<A0>::value) return r1;
      }
      A0 r2 = copysign(details::erfinv_kernel<A0>::erfcinv2(w), a00);
      r2 = if_else(test, r1, r2);
      #ifndef BOOST_SIMD_NO_INFINITIES
      r2 = if_else(eq(w, Inf<A0>()), copysign(w, a00), r2);
      #endif
      return r2;
    }
                            ...
                            )
  {
    ...
    NT2_FUNCTOR_CALL(1)
    {
      ...
      w -= if_else(test1, Three125, if_else(test2, Three25, Five<A0>()));
      ...
      if(nb1 > 0)
      {
        A0 r2 = a00*details::erfinv_kernel<A0>::erfinv2(w);
        r1 = nt2::if_else(test1, r1, r2);
        nb += nb1;
        if(nb >= meta::cardinal_of<A0>::value) return r1;
      }
      r1 = if_else(test2, r1, a00*details::erfinv_kernel<A0>::erfinv3(w));
      #ifndef BOOST_SIMD_NO_INFINITIES
      return if_else(eq(w, Inf<A0>()), copysign(w, a00), r1);
      #else
      return r1;
      #endif
    }
