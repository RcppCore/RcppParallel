  ...
  namespace tag
  {
   /*!
     ...
     @par Models:
        Hierarchy
   **/
    struct logical_notand_ : ext::elementwise_<logical_notand_>
    {
      /// @brief Parent hierarchy
      typedef ext::elementwise_<logical_notand_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_logical_notand_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
   template<class Site, class... Ts>
   BOOST_FORCEINLINE generic_dispatcher<tag::logical_notand_, Site> dispatching_logical_notand_(adl_helper, boost::dispatch::meta::unknown_<Site>, boost::dispatch::meta::unknown_<Ts>...)
   {
     return generic_dispatcher<tag::logical_notand_, Site>();
   }
   template<class... Args>
   struct impl_logical_notand_;
  }
  /*!
