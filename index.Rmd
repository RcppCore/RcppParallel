...
RcppParallel provides the `parallelFor` and `parallelReduce` functions however the TBB library includes a wealth of other tools for parallelization. The motivation for `parallelFor` and `parallelReduce` is compatibility. You can write a single algorithm that uses TBB on Windows, OS X, and Linux but falls back to a more naive implementation based on threads on other platforms.

If however you are okay with targeting only Windows, OS X, and Linux, you can use TBB directly and bypass `parallelFor` and `parallelReduce`. Note that if you are doing this within an R package you plan on submitting to CRAN you should also provide a fallback serial implementation so the package still compiles on other platforms (details on doing this are in the *Compatibility* section below).
