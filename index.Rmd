...
In addition to a robust concurrent programming model, parallelization using TBB can achieve significantly better performance than traditional use of threads or even [OpenMP](http://openmp.org/wp/). This is accomplished via dynamic optimization around grain size (which affects locality of reference and therefore cache hit rates) as well as work stealing (detecting idle threads and pushing work to them).
